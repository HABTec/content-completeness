...
dhis2.util.namespace('dhis2.contentCompleteness');
dhis2.util.namespace('dhis2.rd');

// whether current user has any organisation units
dhis2.contentCompleteness.emptyOrganisationUnits = false;
...
dhis2.contentCompleteness.store = null;
dhis2.rd.metaDataCached = dhis2.rd.metaDataCached || false;
dhis2.contentCompleteness.memoryOnly = $('html').hasClass('ie7') || $('html').hasClass('ie8');
var adapters = [];
if( dhis2.contentCompleteness.memoryOnly ) {
    adapters = [ dhis2.storage.InMemoryAdapter ];
} else {
    adapters = [ dhis2.storage.IndexedDBAdapter, dhis2.storage.DomLocalStorageAdapter, dhis2.storage.InMemoryAdapter ];
}

dhis2.contentCompleteness.store = new dhis2.storage.Store({
    name: 'dhis2cr',
    ...
    if (loggedIn)
    {
        ...
        OfflineDataValueService.hasLocalData().then(function(localData){
            if(localData){
                ...
                if (dhis2.contentCompleteness.emptyOrganisationUnits) {
...
function downloadMetaData( url )
{
    ...
    return dhis2.contentCompleteness.store.open()
...
function getUserAccessibleDataSet(){
    return dhis2.metadata.getMetaObject(null, 'ACCESSIBLE_DATASETS', DHIS2URL + '/api/dataSets.json', 'fields=id,access[data[read,write]]&paging=false', 'sessionStorage', dhis2.contentCompleteness.store);
}
...
function getSystemSetting(){
    ...
    return dhis2.metadata.getMetaObject(null, 'SYSTEM_SETTING', DHIS2URL + '/api/systemSettings', 'key=keyGoogleMapsApiKey&key=keyMapzenSearchApiKey&key=keyCalendar&key=keyDateFormat&key=multiOrganisationUnitForms', 'sessionStorage', dhis2.contentCompleteness.store);
}

function getPeriodTypes(){
    return dhis2.metadata.getMetaObjects('periodTypes', 'periodTypes', DHIS2URL + '/api/periodTypes', 'fields=name,frequencyOrder', 'idb', dhis2.contentCompleteness.store);
}
...
function filterMissingDataElementGroups( objs ){
    return dhis2.metadata.filterMissingObjIds('dataElementGroups', dhis2.contentCompleteness.store, objs);
}

function getDataElementGroups( ids ){
    return dhis2.metadata.getBatches( ids, batchSize, 'dataElementGroups', 'dataElementGroups', DHIS2URL + '/api/dataElementGroups.json', 'paging=false&fields=id,displayName,code,dataElements,attributeValues[value,attribute[id,name,valueType,code]] ','idb', dhis2.contentCompleteness.store, dhis2.metadata.processObject);
}
...
function filterMissingDataSets( objs ){
    return dhis2.metadata.filterMissingObjIds('dataSets', dhis2.contentCompleteness.store, objs);
}
...
function filterMissingOrganisationUnitGroupSets( objs ){
    return dhis2.metadata.filterMissingObjIds('organisationUnitGroupSets', dhis2.contentCompleteness.store, objs);
}

function getOrganisationUnitGroupSets( ids ){
    return dhis2.metadata.getBatches( ids, batchSize, 'organisationUnitGroupSets', 'organisationUnitGroupSets', DHIS2URL + '/api/organisationUnitGroupSets.json', 'paging=false&fields=id,displayName,version,organisationUnitGroups[id,displayName]', 'idb', dhis2.contentCompleteness.store, dhis2.metadata.processObject);
}
...
function filterMissingOptionSets( objs ){
    return dhis2.metadata.filterMissingObjIds('optionSets', dhis2.contentCompleteness.store, objs);
}

function getOptionSets( ids ){
    return dhis2.metadata.getBatches( ids, batchSize, 'optionSets', 'optionSets', DHIS2URL + '/api/optionSets.json', 'paging=false&fields=id,displayName,version,valueType,attributeValues[value,attribute[id,name,valueType,code]],options[id,displayName,code]', 'idb', dhis2.contentCompleteness.store, dhis2.metadata.processObject);
}